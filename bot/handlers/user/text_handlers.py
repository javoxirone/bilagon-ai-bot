...
def format_markdown_for_telegram(text):
    """Format markdown for Telegram: Convert headings (#) to bold uppercase text."""
...
def format_markdown_for_telegram_2(text):
    """
    Format markdown text to be compatible with Telegram's MarkdownV2 format.
    Handles headings, bold, italic, code blocks, and proper escaping.
    """
    # First, escape special characters in the text
    # Telegram requires escaping: '_', '*', '[', ']', '(', ')', '~', '`', '>', '#', '+', '-', '=', '|', '{', '}', '.', '!'
    special_chars = ['_', '*', '[', ']', '(', ')', '~', '`', '>', '#', '+', '-', '=', '|', '{', '}', '.', '!']

    # Process code blocks first (to preserve their content from other formatting)
    ...
    # Now escape special characters in the main text (excluding placeholders)
    for char in special_chars:
        text = text.replace(char, f'\\{char}')

    # Process headings (convert to bold text with appropriate sizing)
    text = re.sub(r'#{4,}\s+(.+)', r'*\1*', text)  # h4+ becomes just bold
    text = re.sub(r'###\s+(.+)', r'*\1*', text)  # h3 becomes bold
    text = re.sub(r'##\s+(.+)', r'*\1*\n', text)  # h2 becomes bold with line break
    text = re.sub(r'#\s+(.+)', r'*\1*\n\n', text)  # h1 becomes bold with double line break

    # Handle other formatting
    # Note: We've already escaped * characters, so we need to handle
    # the properly escaped versions for bold/italic
    text = re.sub(r'\\\*\\\*(.+?)\\\*\\\*', r'*\1*', text)  # Bold
    text = re.sub(r'\\\*(.+?)\\\*', r'_\1_', text)  # Italic
    text = re.sub(r'\_\_(.+?)\_\_', r'_\1_', text)  # Underline (convert to italic)

    # Handle inline code (replace with Telegram's format)
    text = re.sub(r'`([^`]+)`', r'`\1`', text)

    # Restore code blocks (with Telegram's single backtick format for multiline code)
    for placeholder, code_block in code_block_placeholders.items():
        # Use Telegram's single-backtick format for code blocks
        formatted_code = f"`{code_block}`"
