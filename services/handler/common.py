...
async def handle_saved_conversation(telegram_id: int, user_request_message: str, status_message_id: int) -> None:
    ...
    response_generator = get_text_response_with_context(telegram_id, conversation_list, user_language)
...
async def handle_incognito_conversation(telegram_id: int, user_request_message: str, status_message_id: int) -> None:
    user_language = get_user_language(telegram_id)
    response_generator = get_text_response_in_incognito_mode(telegram_id, user_request_message, user_language)
